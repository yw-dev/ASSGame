...
namespace ShooterGameInstanceState
{
	...
	extern const FName ConnectingServer;
	extern const FName PawnGuide;
...
public:
	...
	bool TravelLevel(const FString& InTravelURL);
	bool HostGame(ULocalPlayer* LocalPlayer, const FString& GameType, const FString& InTravelURL);
	bool ConnectionServer(ULocalPlayer* LocalPlayer, const FString& GameType, const FString& InTravelURL);
...
private:
	...
	UPROPERTY(config)
	FString ConnectServerMap;

	UPROPERTY(config)
	FString PawnGuideMap;
	/*
	TSubclassOf<UUserWidget> ConnectServerWidgetClass;

	UUserWidget ConnectServerWidgetIns;

	TSubclassOf<UUserWidget> PawnGuideWidgetClass;

	UUserWidget PawnGuideWidgetIns;
	*/
	FName CurrentState;
	FName PendingState;
	...
	/** Welcome menu UI (for consoles) */
	TSharedPtr<FShooterLogin> LoginUI;

	/** Welcome menu UI (for consoles) */
	TSharedPtr<FShooterPawnGuide> PawnGuideUI;
	...
	void BeginConnectServerState();
	void BeginPawnGuideState();
	...
	void EndConnectServerState();
	void EndPawnGuideState();
...
public:
	...
	UPROPERTY(EditAnywhere, BlueprintReadOnly, Category = AssetSource)
	...
	/** Number of slots for each type of item */
	//UPROPERTY(EditAnywhere, BlueprintReadOnly, Category = AssetSource)
	//TMap<FPrimaryAssetType, TArray<class UShooterItem*>> AssetSourceMap;
	...
	void AddDataSource(UShooterSaveGame* SaveGame, bool bRemoveExtra = false);

	UFUNCTION(BlueprintCallable, Category = Inventory)
	void AddPawnSource(UShooterSaveGame* SaveGame, bool bRemoveExtra = false);

	/** Returns true if this is a valid inventory slot */
	UFUNCTION(BlueprintCallable, Category = Inventory)
	bool IsValidItemSlot(FShooterItemSlot ItemSlot) const;
	...
	UShooterSaveGame* GetCurrentSaveGame();
