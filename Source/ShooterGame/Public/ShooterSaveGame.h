...
namespace EShooterSaveGameVersion
{
	enum type
	{
		// Initial version
...
public:
	...
	TMap<FPrimaryAssetId, FShooterItemData> AssetDataSource;

	/** Map of items to item data */
	UPROPERTY(VisibleAnywhere, BlueprintReadWrite, Category = SaveGame)
	TMap<FPrimaryAssetId, FShooterItemData> PawnAssetSource;

	/** Map of slotted items */
	UPROPERTY(VisibleAnywhere, BlueprintReadWrite, Category = SaveGame)
	TMap<FShooterItemSlot, FPrimaryAssetId> SlottedItems;
