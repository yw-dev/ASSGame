...
bool AShooterAIController::HasWeaponLOSToEnemy(AActor* InEnemyActor, const bool bAnyEnemy) const
{
	...
	GetWorld()->LineTraceSingleByChannel(Hit, StartLocation, EndLocation, COLLISION_TC_WEAPON, TraceParams);
...
{
	AShooterBot* MyBot = Cast<AShooterBot>(GetPawn());
	AShooterWeaponBase* MyWeapon = MyBot ? MyBot->GetWeapon() : NULL;
	if (MyWeapon == NULL)
	{
		return;
	}
	...
	if ( Enemy && ( Enemy->IsAlive() ) && ( MyWeapon->CanFire() == true ) )
	{
...
void AShooterAIController::CheckAmmo(const class AShooterWeaponBase* CurrentWeapon)
{
	if (CurrentWeapon && BlackboardComp)
	{
		if (CurrentWeapon->GetCurrentWeaponType()!= EShooterWeaponType::Gun)
		{
			return;
		}
		AShooterRangedWeapon* RangedWeapon = (AShooterRangedWeapon*)CurrentWeapon;
		const int32 Ammo = RangedWeapon->GetCurrentAmmo();
