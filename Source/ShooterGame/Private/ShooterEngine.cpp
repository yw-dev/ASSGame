...
void UShooterEngine::HandleNetworkFailure(UWorld *World, UNetDriver *NetDriver, ENetworkFailure::Type FailureType, const FString& ErrorString)
{
	...
	UE_LOG(LogTemp, Warning, TEXT("%s"), *FString::Printf(TEXT("NetDriverName : %s"), *NetDriverName.ToString()));
		...
		//if (World)
		{
			...
			if (NetDriver)
			{
				switch (FailureType)
				{
					...
					{
						UE_LOG(LogTemp, Warning, TEXT("ENetworkFailure::FailureReceived"));
					...
					{
						UE_LOG(LogTemp, Warning, TEXT("ENetworkFailure::PendingConnectionFailure"));
					...
					{
						UE_LOG(LogTemp, Warning, TEXT("ENetworkFailure::ConnectionLost/ConnectionTimeout"));
						UShooterGameInstance* const GI = Cast<UShooterGameInstance>(GameInstance);
						if (GI && NetDriver->GetNetMode() == NM_Client)
						{
					...
					default:
						UE_LOG(LogTemp, Warning, TEXT("ENetworkFailure::NetDriverAlreadyExists/NetDriverCreateFailure/OutdatedClient/OutdatedServer/default"));
						break;
				}
			}
		}
