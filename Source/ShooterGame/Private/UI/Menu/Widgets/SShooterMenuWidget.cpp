...
{
	UE_LOG(LogTemp, Warning, TEXT("MenuWidget::BuildLeftPanel()"));
	...
	for(int32 i = 0; i < CurrentMenu.Num(); ++i)
	{
		if (CurrentMenu[i]->bVisible)
		{
			...
			{
				UE_LOG(LogTemp, Warning, TEXT("MenuWidget::BuildLeftPanel( Standard )"));
			...
			{
				UE_LOG(LogTemp, Warning, TEXT("MenuWidget::BuildLeftPanel( MultiChoice )"));
				TmpWidget = SAssignNew(CurrentMenu[i]->Widget, SShooterMenuItem)
					.PlayerOwner(PlayerOwner)
					.OnClicked(this, &SShooterMenuWidget::ButtonClicked, i)
					.Text(CurrentMenu[i]->GetText() )
			...
			{
				UE_LOG(LogTemp, Warning, TEXT("MenuWidget::BuildLeftPanel( CustomWidget )"));
			...
			if (TmpWidget.IsValid())
			{
				...
				LeftBox->AddSlot().HAlign(HAlign_Fill).VAlign(VAlign_Fill).AutoHeight()
				[
...
{
	UE_LOG(LogTemp, Warning, TEXT("MenuWidget::BuildRightPanel()"));
	...
	{
		UE_LOG(LogTemp, Warning, TEXT("MenuWidget::BuildRightPanel( bVisible )"));
		...
		//{
			...
			{
				UE_LOG(LogTemp, Warning, TEXT("MenuWidget::BuildRightPanel( Standard )"));
				TmpWidget = SAssignNew(NextMenu[i]->Widget, SShooterMenuItem)
					.PlayerOwner(PlayerOwner)
					.Text(NextMenu[i]->GetText())
					.InactiveTextAlpha(1.f)
					.bIsMultichoice(false);
			}
			else if (NextMenu[i]->MenuItemType == EShooterMenuItemType::MultiChoice)
			{
				UE_LOG(LogTemp, Warning, TEXT("MenuWidget::BuildRightPanel( MultiChoice )"));
				TmpWidget = SAssignNew(NextMenu[i]->Widget, SShooterMenuItem)
					.PlayerOwner(PlayerOwner)
					.Text(NextMenu[i]->GetText() )
					.InactiveTextAlpha(1.f)
			...
			else if (NextMenu[i]->MenuItemType == EShooterMenuItemType::CustomWidget)
			{
				if (NextMenu[i]->WidgetType == EShooterCustomWidget::Save)
				{
					UE_LOG(LogTemp, Warning, TEXT("MenuWidget::BuildRightPanel( Save )"));
					TmpWidget = SAssignNew(NextMenu[i]->CustomWidget, SShooterSessionsWidget).PlayerOwner(PlayerOwner);
				}
				else if (NextMenu[i]->WidgetType == EShooterCustomWidget::Options)
				{
					UE_LOG(LogTemp, Warning, TEXT("MenuWidget::BuildRightPanel( Options )"));
					TmpWidget = SAssignNew(NextMenu[i]->CustomWidget, SShooterOptionWidget).PlayerOwner(PlayerOwner);
				}
				else if (NextMenu[i]->WidgetType == EShooterCustomWidget::Server)
				{
					UE_LOG(LogTemp, Warning, TEXT("MenuWidget::BuildRightPanel( Server )"));
					TmpWidget = SAssignNew(NextMenu[i]->CustomWidget, SShooterSessionsWidget).PlayerOwner(PlayerOwner);
				}
				else if (NextMenu[i]->WidgetType == EShooterCustomWidget::Main)
				{
					UE_LOG(LogTemp, Warning, TEXT("MenuWidget::BuildRightPanel( Main )"));
					TmpWidget = SAssignNew(NextMenu[i]->CustomWidget, SShooterPawnGuideWidget).PlayerOwner(PlayerOwner);
				}
			}
			if(TmpWidget.IsValid())
			{
				RightBox->AddSlot().HAlign(HAlign_Fill).VAlign(VAlign_Fill)
				[
					TmpWidget.ToSharedRef()
				];
			}
...
{
	return CurrentMenuTitle.IsEmpty() ? FLinearColor::Transparent : FLinearColor::Black;
}
