...
void FShooterIngameMenu::Construct(ULocalPlayer* _PlayerOwner)
{
	...
	if (!GameMenuWidget.IsValid())
	{
		...
		//MainMenuItem = MenuHelper::AddMenuItem(RootMenuItem,LOCTEXT("Main Menu", "主菜单"));
		...
		// HOST menu option
		MainMenuItem = MenuHelper::AddMenuItem(RootMenuItem, LOCTEXT("Server", "联网"));

		// Server list widget that will be called up if appropriate
		MenuHelper::AddCustomWidget(MainMenuItem, SAssignNew(SessionListWidget, SShooterSessionsWidget).OwnerWidget(GameMenuWidget).PlayerOwner(PlayerOwner), EShooterCustomWidget::Server);

		// menu option
		MainMenuItem = MenuHelper::AddMenuItem(RootMenuItem, LOCTEXT("Option", "选项"));

		// Server list widget that will be called up if appropriate
		MenuHelper::AddCustomWidget(MainMenuItem, SAssignNew(OptionWidget, SShooterOptionWidget).OwnerWidget(GameMenuWidget).PlayerOwner(PlayerOwner), EShooterCustomWidget::Options);

		// Server list widget that will be called up if appropriate
		//MenuHelper::AddCustomMenuItem(JoinServerItem, SAssignNew(ServerListWidget, SShooterSessionsWidget).OwnerWidget(MenuWidget).PlayerOwner(GetPlayerOwner()));
		/*
...
//#if !SHOOTER_CONSOLE_UI
		MenuHelper::AddMenuItemSP(RootMenuItem, LOCTEXT("Quit", "退出"), this, &FShooterIngameMenu::OnUIQuit);
//#endif

		GameMenuWidget->CurrentMenuTitle = LOCTEXT("Titles", "主菜单");
