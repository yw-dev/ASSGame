...
void FShooterMainMenu::Construct(TWeakObjectPtr<UShooterGameInstance> _GameInstance, TWeakObjectPtr<ULocalPlayer> _PlayerOwner)
{
	...
	{		
		...
		TSharedPtr<FShooterMenuItem> MenuItem;
		// Main menu option
		MenuItem = MenuHelper::AddMenuItem(RootMenuItem, LOCTEXT("Play", "开始"));

		// Main list widget that will be called up if appropriate
		MenuHelper::AddCustomWidget(MenuItem, SAssignNew(PawnGuideWidget, SShooterPawnGuideWidget).OwnerWidget(MenuWidget).PlayerOwner(GetPlayerOwner()), EShooterCustomWidget::Main);

		// Server menu option
		MenuItem = MenuHelper::AddMenuItem(RootMenuItem, LOCTEXT("Server", "联机"));

		// Server list widget that will be called up if appropriate
		MenuHelper::AddCustomWidget(MenuItem, SAssignNew(SessionListWidget, SShooterSessionsWidget).OwnerWidget(MenuWidget).PlayerOwner(GetPlayerOwner()), EShooterCustomWidget::Server);

		// menu option
		MenuItem = MenuHelper::AddMenuItem(RootMenuItem, LOCTEXT("Option", "选项"));

		// Server list widget that will be called up if appropriate
		MenuHelper::AddCustomWidget(MenuItem, SAssignNew(OptionWidget, SShooterOptionWidget).OwnerWidget(MenuWidget).PlayerOwner(GetPlayerOwner()), EShooterCustomWidget::Options);
		...
		{
			TSharedPtr<FShooterMenuItem> HelpSubMenu = MenuHelper::AddMenuItem(RootMenuItem, LOCTEXT("Help", "帮助"));
			HelpSubMenu->OnConfirmMenuItem.BindStatic([](){ FSlateApplication::Get().ShowSystemHelp(); });
		}

		// QUIT option (for PC)
//#if !SHOOTER_CONSOLE_UI
		MenuHelper::AddMenuItemSP(RootMenuItem, LOCTEXT("Quit", "退出"), this, &FShooterMainMenu::OnUIQuit);
//#endif

		MenuWidget->CurrentMenuTitle = LOCTEXT("Title","网络游戏实验");
...
{
	PlayerOwner->GetPlayerController(PlayerOwner->GetWorld())->bShowMouseCursor = true;
	//PlayerOwner->bEnableMouseOverEvents = true;
	PlayerOwner->GetPlayerController(PlayerOwner->GetWorld())->bEnableClickEvents = true;
