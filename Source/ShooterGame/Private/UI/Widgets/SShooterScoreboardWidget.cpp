...
void SShooterScoreboardWidget::Construct(const FArguments& InArgs)
{
	...
	ScoreboardTint = FLinearColor(0.0f,0.0f,0.0f,0.75f);
	ScoreBoxWidth = 160.0f;
	...
	ScoreboardGrid->AddSlot().HAlign(HAlign_Fill).VAlign(VAlign_Fill).AutoHeight()
	[
			...
			[
				...
				[
					...
					[
						...
						.TextStyle(FShooterStyle::Get(), "ShooterGame.DefaultScoreboard.Row.TitleTextStyle")
...
TSharedRef<SWidget> SShooterScoreboardWidget::MakePlayerTableView() const
{
	/** holds player info rows */
	TSharedPtr<SVerticalBox> ScoreDataTable = SNew(SVerticalBox);
	...
	for (uint8 TeamNum = 0; TeamNum < PlayerStateMaps.Num(); TeamNum++)
	{
		...
		if (PlayerStateMaps.Num() > 1 && PlayerStateMaps[TeamNum].Num() > 0)
		{
			...
			//	];
			ScoreDataTable->AddSlot().Padding(FMargin(10, 15, 10, 0)).AutoHeight()
				[
					MakeTotalsRow(TeamNum)
				];
			ScoreDataTable->AddSlot().AutoHeight()
				[
					SNew(SBorder)
					.BorderImage(&MenuStyle->RightBackgroundBrush)
					.BorderBackgroundColor(FLinearColor(1.0f, 1.0f, 1.0f, 1.0f))
			...
			//ScoreDataPanel->AddSlot().AutoHeight()
			//	[
			//		MakePlayerTitleRow()
			//	];
		}
	...
	return ScoreDataTable.ToSharedRef();
}

TSharedRef<SWidget> SShooterScoreboardWidget::MakePlayerTitleRow() const
{
	TSharedPtr<SHorizontalBox> HeaderCols;

	//Padding in the header row (invisible) for speaker icon
	HeaderCols = SNew(SHorizontalBox)
		+ SHorizontalBox::Slot().Padding(NORM_PADDING).AutoWidth()
		[
			SNew(SBox)
			.HeightOverride(35)
			.WidthOverride(35)
	...
	+ SHorizontalBox::Slot().Padding(NORM_PADDING).VAlign(VAlign_Fill).HAlign(HAlign_Fill).AutoWidth()
		[
			SNew(SBox)
			.WidthOverride(300)
			.HeightOverride(35)
			[
				...
				[
					SNew(STextBlock)
					.Text(LOCTEXT("PlayerNameColumn", "玩家"))
					.TextStyle(FShooterStyle::Get(), "ShooterGame.DefaultScoreboard.Row.TitleTextStyle")
				]
			]
		];
	...
	for (uint8 ColIdx = 0; ColIdx < Columns.Num(); ColIdx++)
	{
		...
		HeaderCols->AddSlot().Padding(NORM_PADDING).VAlign(VAlign_Fill).HAlign(HAlign_Fill).AutoWidth()
			[
				SNew(SBox)
...
TSharedRef<SWidget> SShooterScoreboardWidget::MakeTotalsRow(uint8 TeamNum) const
{
	...
	[
		...
		[
			SNew(SHorizontalBox)
			+ SHorizontalBox::Slot().VAlign(VAlign_Fill).HAlign(HAlign_Left)
			[
				SNew(SBox)
				.HeightOverride(35)
...
TSharedRef<SWidget> SShooterScoreboardWidget::MakePlayerRow(const FTeamPlayer& TeamPlayer) const
{
	...
	PlayerRow->AddSlot().Padding(NORM_PADDING).VAlign(VAlign_Fill).HAlign(HAlign_Fill).AutoWidth()
	[
		SNew(SBox)
		.WidthOverride(300)
		.HeightOverride(35)
	...
	{
		PlayerRow->AddSlot()
		.Padding(NORM_PADDING).HAlign(HAlign_Fill).VAlign(VAlign_Fill).AutoWidth()
		[
			SNew(SBox)
			.WidthOverride(ScoreBoxWidth)
			[
