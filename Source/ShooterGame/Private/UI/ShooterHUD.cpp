...
void AShooterHUD::DrawWeaponHUD()
{
	...
	AShooterWeaponBase* MyWeapon = MyPawn->GetWeapon();
	if (MyWeapon)
	{
		...
		//SECONDARY WEAPON
		AShooterWeaponBase* SecondaryWeapon = NULL;

		for (const TPair<FShooterItemSlot, class AShooterWeaponBase*>& Pair : MyPawn->GetInventoryList())
		{
			if (Pair.Value != MyWeapon)
			{
				SecondaryWeapon = Pair.Value;
				break;
			}
		}/*
...
void AShooterHUD::DrawCrosshair()
{
	...
	if (PCOwner)
	{
			...
			&& (Pawn->IsTargeting() || (!Pawn->IsTargeting() && !Pawn->GetWeapon()->bHideCrosshairWhileNotAiming)))
		{
			...
			AShooterWeaponBase* MyWeapon = Pawn->GetWeapon();
			const float CurrentTime = GetWorld()->GetTimeSeconds();
