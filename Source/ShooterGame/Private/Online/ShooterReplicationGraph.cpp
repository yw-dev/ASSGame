...
*		
*	Dependent Actors (AShooterWeaponBase)
*		
*		Replication Graph introduces a concept of dependent actor replication. This is an actor (AShooterWeaponBase) that only replicates when another actor replicates (Pawn). I.e, the weapon
...
void UShooterReplicationGraph::InitGlobalActorClassSettings()
{
	...
	AddInfo( AShooterWeaponBase::StaticClass(),						EClassRepNodeMapping::NotRouted);				// Handled via DependantActor replication (Pawn)
...
void UShooterReplicationGraphNode_AlwaysRelevant_ForConnection::GatherActorListsForConnection(const FConnectionGatherActorListParameters& Params)
{
	...
	if (AShooterPlayerController* PC = Cast<AShooterPlayerController>(Params.Viewer.InViewer))
	{
		...
		if (AShooterCharacter* Pawn = Cast<AShooterCharacter>(PC->GetPawn()))
		{
			...
			for (const TPair<FShooterItemSlot, class AShooterWeaponBase*>& Pair : Pawn->GetInventoryList())
			{
				AShooterWeaponBase* Weapon = Pair.Value;
				if (Weapon)
				{
			...
			{
				AShooterWeaponBase* Weapon = Pawn->FindWeapon(i);
				if (Weapon)
				{
					ReplicationActorList.ConditionalAdd(Weapon);
				}
