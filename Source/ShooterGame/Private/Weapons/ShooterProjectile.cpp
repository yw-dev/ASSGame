...
AShooterProjectile::AShooterProjectile(const FObjectInitializer& ObjectInitializer) : Super(ObjectInitializer)
{
	...
	CollisionComp->SetCollisionObjectType(COLLISION_OC_PROJECTILE);
...
void AShooterProjectile::OnRep_Exploded()
{
	...
	if (!GetWorld()->LineTraceSingleByChannel(Impact, StartTrace, EndTrace, COLLISION_OC_PROJECTILE, FCollisionQueryParams(SCENE_QUERY_STAT(ProjClient), true, Instigator)))
	{
		// failsafe
