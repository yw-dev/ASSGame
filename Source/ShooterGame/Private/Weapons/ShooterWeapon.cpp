...
AShooterWeapon::AShooterWeapon(const FObjectInitializer& ObjectInitializer) : Super(ObjectInitializer)
{
	...
	CapsuleCollision->SetRelativeLocation(FVector(0.f, 0.f, 45.f));
	...
	Mesh1P->SetupAttachment(CapsuleCollision);
	...
	Mesh3P->SetCollisionResponseToChannel(COLLISION_WEAPON_SWORD, ECR_Overlap);
	Mesh3P->SetCollisionResponseToChannel(COLLISION_MARGIC, ECR_Block);
...
void AShooterWeapon::OnEquip(const AShooterWeaponBase* LastWeapon)
{
	AttachMeshToPawn();
...
EShooterWeaponType AShooterWeapon::GetActiveWeaponType() const
{
	//UE_LOG(LogTemp, Warning, TEXT("ActiveWeaponType = %u"), (uint32)WeaponConfig.ActiveWeaponType);
	return WeaponConfig.ActiveWeaponType;
}*/
